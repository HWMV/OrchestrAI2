...
class WebtoonEditor(CrewAgent):
    ...
    def execute_task(self, task: Any, context: Dict[str, Any] = None, tools: List[Any] = None) -> str:
        ...
        result = loop.run_until_complete(self._execute_task_async(task, context, tools))
        return result

    async def _execute_task_async(self, task: Any, context: Dict[str, Any] = None, tools: List[Any] = None) -> str:
...
class WebtoonArtist(CrewAgent):
    ...
    def generate_dalle_image(self, prompt, max_retries=3, delay=2):
        for attempt in range(max_retries):
            try:
                ...
                logger.error(f"Error generating image (attempt {attempt + 1}/{max_retries}): {str(e)}")
    ...
    async def execute_task(self, task: Any, context: Dict[str, Any] = None, **kwargs) -> str:
        ...
        # webtoon_drawing 태스크 실행 (DALL-E 프롬프트 생성)
        dalle_prompt = super().execute_task(task, context=context, **kwargs)
